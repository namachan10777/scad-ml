...
(** Oftentimes, we would just like to define our paths without colinear points,
    ...
    more fine sampling, and in those times we can reach for {{!Scad_ml.Path3.resample}
    [Path3.resample]} or {{!Scad_ml.Path3.subdivide} [Path3.subdivide]}. For
...
(** Lets visualize our original [path], and the [resampled] points
    with {{!Scad_ml.Path3.show_points} [Path3.show_points]} to get a sense of

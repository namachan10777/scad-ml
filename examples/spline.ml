...
(** Mark our [control] points with the debugging helper
    {{!Scad_ml.Path2.show_points} [Path2.show_points]} for reference. We don't
...
(** Fit a cubic spline to the [control] points, and interpolate [fn] points
    along it using the helper {{!Scad_ml.Path2.cubic_spline}
    [Path2.cubic_spline]} (lower level functions available in the
    {{!Scad_ml.CubicSpline} [CubicSpline]} module), then sweep a [rectangle]
    along it. *)
let line =

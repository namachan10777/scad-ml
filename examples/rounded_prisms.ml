...
(** If you are in need of extrusions with more continuous curvature than what can be
    achieved by {{!Scad_ml.Mesh.Cap.t} [Mesh.Cap.t]} specified sweep caps,
    there are the {{!Scad_ml.Mesh.linear_prism} [Mesh.linear_prism]} and
    {{!Scad_ml.Mesh.prism} [Mesh.prism]}.
...
(** The {{!Scad_ml.Mesh.prism} [Mesh.prism]} function provides us with a bit
    ...
    y-axis does not break the requirements of {{!Scad_ml.Mesh.prism}

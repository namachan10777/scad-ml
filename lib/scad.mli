(** The core module of this library. Containing the core type, and all
    operations upon it.
...
type 'space t
(** This GADT allows scads to be tagged as 2D or 3D, restricting usage of
    functions that should only apply to one or the other, and preventing mixing
...
val cylinder :
  ?center:bool -> ?fa:float -> ?fs:float -> ?fn:int -> float -> float -> d3
(** [cylinder ?center ?fa ?fs ?fn radius height]

    Creates a cylinder centered about the z axis. When center is true, it will
    also be centered vertically, otherwise the base will sit upon the XY plane. *)
...
(** [square ?center dimensions]
    ...
    [dimensions]. When [?center] is true the square is centered on the origin. *)
...
val polygon :
  ?convexity:int -> ?paths:int list list -> (float * float) list -> d2
...
(** [text ?size ?font ?halign ?valign ?spacing ?direction ?language ?script ?fn str]
    ...
    - [?font] is the name of the font that should be used. This is not the name
      ...
      fontconfig library).
    - [?halign] and [?valign] set the horizontal and vertical alignments of the
      text. Defaults are [Left] and [Baseline] respectively.
    - [?spacing] gives a factor by which to increase/decrease the character
      spacing. A value of 1.0 would result in normal spacing.
...
(** [rotate r t]
    ...
    Performs an Euler rotation (x -> y -> z) on [t] with the angles [r] (in
    radians). *)
...
(** [offset ?chamfer offset t]
    ...
    Generates a new 2D interior or exterior outline from an existing outline
    [t].
...
(** {1 Boolean Combination}
    ...
    list inputs are expected, then use the appropriate [_2d] or [_3d] variant. *)
...
(** [union ts]
    ...
    The solution to this is to use a small value called an epsilon when merging
    adjacent faces to guarantee that they overlap, like so:

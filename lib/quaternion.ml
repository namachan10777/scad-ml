...
let basic_op op (x1, y1, z1, w1) (x2, y2, z2, w2) =
...
let alignment v1 v2 =
  ...
  if dp > 0.999999 (* already parallel *) then id
  else if dp < -0.999999 (* opposite *) then
    let x_cross = Vec3.cross (1., 0., 0.) v1 in
    let axis =
      Vec3.normalize
      @@
      if Vec3.norm x_cross < 0.000001 then Vec3.cross (0., 1., 0.) v1
      else x_cross
    in
    make axis Float.pi
  else

...
type scad =
  ...
  | Square of { size : float * float; center : bool }
  | Circle of {
      r : float;
      fa : float option;
      fs : float option;
  ...
  | Text of Text.t
  | Color of { src : scad; color : Color.t; alpha : float option }
  ...
  | Projection of { src : scad; cut : bool }
  | LinearExtrude of {
      src : scad;
      height : float option;
      center : bool;
  ...
  | Import of { file : string; convexity : int; dxf_layer : string option }

type 'space t = D2 : scad -> two_d t | D3 : scad -> three_d t
type d2 = two_d t
type d3 = three_d t

let d2 scad = D2 scad
let d3 scad = D3 scad
let unpack : type a. a t -> scad = function D2 scad -> scad | D3 scad -> scad

let map : type a. (scad -> scad) -> a t -> a t =
 fun f -> function D2 scad -> D2 (f scad) | D3 scad -> D3 (f scad)
...
let polygon ?(convexity = 10) ?paths points =
  d2 @@ Polygon { points; paths; convexity }

let text ?size ?font ?halign ?valign ?spacing ?direction ?language ?script ?fn
    str =
  d2
  @@ Text
       {
...
let rotate_about_pt r p t =
  translate p t |> rotate r |> translate (Vec3.negate p)
...
let quaternion q =
  map (fun scad -> MultMatrix (Quaternion.to_multmatrix q, scad))

let quaternion_about_pt q p t =
  translate p t |> quaternion q |> translate (Vec3.negate p)
...
let empty_message n =
  Printf.sprintf
    "List must be non-empty. Use %s_2d or %s_3d if empty lists are expected." n
    n
...
let linear_extrude ?height ?(center = false) ?(convexity = 10) ?twist
    ?(slices = 20) ?(scale = (1.0, 1.0)) ?(fn = 16) (D2 src) =
  d3
...
let offset ?(chamfer = false) offset (D2 src) =
  d2 @@ Offset { src; offset; chamfer }

let import ?dxf_layer ?(convexity = 10) file =
...
let import_2d ?dxf_layer ?convexity file =
  ...
  | Error ext ->
      failwith
        (Printf.sprintf
           "Input file extension %s is not supported for 2D import." ext)
...
let import_3d ?convexity file =
  ...
  | Error ext ->
      failwith
        (Printf.sprintf
           "Input file extension %s is not supported for 3D import." ext)

let color ?alpha color = map (fun src -> Color { src; color; alpha })

let to_string t =
  let value_map f ~default = function Some x -> f x | None -> default
  ...
  and print indent = function
    | Cylinder { r1; r2; h; center; fa; fs; fn } ->
        Printf.sprintf "%scylinder(h=%f, r1=%f, r2=%f, center=%B%s);\n" indent h
          r1 r2 center (string_of_f_ fa fs fn)
    ...
    | Polygon { points; paths; convexity } ->
        Printf.sprintf "%spolygon(points=%s%s, convexity=%d);\n" indent
    ...
    | Text
        ...
        Printf.sprintf "%stext(\"%s\"%s%s%s%s%s%s%s%s%s);\n" indent text
          (maybe_fmt ", size=\"%f\"" size)
          (maybe_fmt ", font=\"%s\"" font)
          (Option.map Text.h_align_to_string halign
          |> maybe_fmt ", halign=\"%s\"")
          (Option.map Text.v_align_to_string valign
          |> maybe_fmt ", valign=\"%s\"")
          (maybe_fmt ", spacing=\"%f\"" spacing)
          (Option.map Text.direction_to_string direction
          |> maybe_fmt ", direction=\"%s\"")
          (maybe_fmt ", language=\"%s\"" language)
          (maybe_fmt ", script=\"%s\"" script)
          (maybe_fmt ", $fn=\"%i\"" fn)
    | Translate (p, scad) ->
        Printf.sprintf "%stranslate(%s)\n%s" indent (Vec3.to_string p)
          (print (indent ^ "\t") scad)
    | Rotate (r, scad) ->
    ...
    | VectorRotate (axis, r, scad) ->
        Printf.sprintf "%srotate(a=%f, v=%s)\n%s" indent (deg_of_rad r)
          (Vec3.to_string axis)
          (print (indent ^ "\t") scad)
    | MultMatrix (mat, scad) ->
        Printf.sprintf "%smultmatrix(%s)\n%s" indent (MultMatrix.to_string mat)
          (print (indent ^ "\t") scad)
    | Union elements ->
    ...
    | Polyhedron { points; faces; convexity } ->
        Printf.sprintf "%spolyhedron(points=%s, faces=%s, convexity=%i);\n"
          indent
          (string_of_list Vec3.to_string points)
          (string_of_list (string_of_list string_of_int) faces)
          convexity
    | Mirror ((x, y, z), scad) ->
        Printf.sprintf "%smirror(v=[%f, %f, %f])\n%s" indent x y z
          (print (indent ^ "\t") scad)
    | Projection { src; cut } ->
        Printf.sprintf "%sprojection(cut=%B){\n%s%s}\n" indent cut
          ...
          indent
    | LinearExtrude
        { src; height; center; convexity; twist; slices; scale = sx, sy; fn } ->
        Printf.sprintf
          "%slinear_extrude(%scenter=%B, convexity=%d, %sslices=%d, scale=[%f, \
           %f], $fn=%d)\n\
           %s"
          indent
    ...
    | RotateExtrude { src; angle; convexity; fa; fs; fn } ->
        Printf.sprintf "%srotate_extrude(%sconvexity=%d%s)\n%s" indent
          (Option.map deg_of_rad angle |> maybe_fmt "angle=%f")
          convexity (string_of_f_ fa fs fn)
          (print (indent ^ "\t") src)
    | Scale (p, scad) ->
        Printf.sprintf "%sscale(%s)\n%s" indent (Vec3.to_string p)
          (print (indent ^ "\t") scad)
    | Resize (p, scad) ->
        Printf.sprintf "%sresize(%s)\n%s" indent (Vec3.to_string p)
          (print (indent ^ "\t") scad)
    | Offset { src; offset; chamfer } ->
        Printf.sprintf "%soffset(%s, chamfer=%B)\n%s" indent
          (match offset with
    ...
    | Color { src; color; alpha } ->
        Printf.sprintf "%scolor(%s%s)\n%s" indent (Color.to_string color)

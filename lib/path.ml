...
module type LINEANGLE = sig
  type line

  val line_angle : line -> line -> float
end

module LineAngle2 = struct
  let line_angle (l1 : Vec2.line) (l2 : Vec2.line) =
    Vec2.(angle (sub l1.b l1.a) (sub l2.b l2.a))
end

module LineAngle3 = struct
  let line_angle (l1 : Vec3.line) (l2 : Vec3.line) =
    let plane = Plane.make l1.a l1.b l2.b in
    Plane.line_angle plane l2
end

module Search
    (V : Vec.S)
    (BT : BallTree.S with type vec := V.t)
    (LA : LINEANGLE with type line := V.line) =
struct
  ...
  let closest_tangent ?(closed = true) ?(offset = V.zero) ~line curve =
    ...
    | p0 :: p1 :: tl ->
      let angle_sign tangent = Float.sign_bit (LA.line_angle line tangent) in

(** [quad ~p1 ~p2 ~p3 t]
    ...
    Quadratic Bézier float interpolation. Throws exception when [t] is outside
    the range of 0 -> 1. *)
val quad : p1:float -> p2:float -> p3:float -> float -> float

(** [cubic ~p1 ~p2 ~p3 ~p4 t]

    Cubic Bézier float interpolation. Throws exception when [t] is outside
    the range of 0 -> 1. *)
...
(** [quad_vec2 ~p1 ~p2 ~p3 t]
    ...
    [p1] when [t = 0.] and ending at [p3] when [t = 1.]. Throws exception when
    [t] is outside the range of 0 -> 1*)
val quad_vec2 : p1:Vec2.t -> p2:Vec2.t -> p3:Vec2.t -> float -> Vec2.t

(** [cubic_vec2 ~p1 ~p2 ~p3 ~p4 t]

    Cubic Bézier function, interpolating a path in 2d space starting from [p1]
    when [t = 0.] and ending at [p4] when [t = 1.]. Throws exception when [t] is
...
(** [cubic_vec3 ~p1 ~p2 ~p3 ~p4 t]
    ...
    Cubic Bézier function, interpolating a path in 3d space starting from [p1]
    when [t = 0.] and ending at [p4] when [t = 1.]. Throws exception when [t] is
    outside the range of 0 -> 1*)
